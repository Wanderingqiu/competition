...
tf.flags.DEFINE_string(
    'checkpoint_path_adv_inception_resnet_v2', 'adv_inception_resnet_v2.ckpt', 'Path to checkpoint for inception network.')

tf.flags.DEFINE_string(
    'checkpoint_path_adv_inception_v3', 'adv_inception_v3.ckpt', 'Path to checkpoint for inception network.')

tf.flags.DEFINE_string(
    'checkpoint_path_ens3_adv_inception_v3', 'ens3_adv_inception_v3.ckpt', 'Path to checkpoint for inception network.')

tf.flags.DEFINE_string(
    'checkpoint_path_ens4_adv_inception_v3', 'ens4_adv_inception_v3.ckpt', 'Path to checkpoint for inception network.')

tf.flags.DEFINE_string(
    'checkpoint_path_ens_adv_inception_resnet_v2', 'ens_adv_inception_resnet_v2.ckpt', 'Path to checkpoint for inception network.')

tf.flags.DEFINE_string(
    'input_dir', 'dataset2', 'Input directory with images.')
...
tf.flags.DEFINE_float(
    'prob', 0.6, 'probability of using diverse inputs.')
tf.flags.DEFINE_integer(
    'image_resize', 350, 'Height of each input images.')
...
def load_target_class(input_dir):
  ...
  with tf.gfile.Open(os.path.join(input_dir, 'dataset2.csv')) as f:
...
def save_images(images, filenames, output_dir):
  ...
  for i, filename in enumerate(filenames):
    ...
    with tf.gfile.Open(os.path.join(output_dir, filename+'.png'), 'wb') as f:
...
def input_diversity(input_tensor):
  ...
  #ret_noise =tf.truncated_normal([10,FLAGS.image_height, FLAGS.image_width,3],stddev=0.2/FLAGS.iterations)
  #ret = ret+ret_noise
  return ret
